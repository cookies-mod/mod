...
public class PartyCommandsFeature {
	...
	private static final Map<Predicate<String>, PartyCommand> commandMap = Map.ofEntries(
			entry(
					command -> command.equals("ptme"), new PartyCommand(
							() -> PartyCommandsFeature.shouldRunCommand("ptme"), (command, message) -> {
			...
			entry(
					command -> command.equals("warp"), new PartyCommand(
							() -> PartyCommandsFeature.shouldRunCommand("warp"), (command, message) -> {
			...
			entry(
					command -> {
						...
						return command.matches("f[1-7]|m[1-7]|t[1-5]");
							...
							() -> PartyCommandsFeature.shouldRunCommand("joininstance"), (command, message) -> {
			...
			entry(
					command -> command.startsWith("dt"),
					new DownTimeCommand(() -> PartyCommandsFeature.shouldRunCommand("dt"))),
			entry(
					command -> command.equals("cf"), new PartyCommand(
							() -> PartyCommandsFeature.shouldRunCommand("cf"), (command, message) -> {
	...
	private static boolean shouldRunCommand(String partyCommand) {
		...
		return PartyCommands.isCommandEnabled(partyCommand) && player.filter(clientPlayerEntity -> PartyUtils.isInParty() && PartyUtils.getPartyLeader()
				.equals(clientPlayerEntity.getUuid())).isPresent();
	}
	...
	private static void handleChat(Text text, boolean b) {
		...
		var command = commandMap.entrySet()
				.stream()
				.filter(commandEntry -> commandEntry.getKey().test(commandString))
				.findFirst();
