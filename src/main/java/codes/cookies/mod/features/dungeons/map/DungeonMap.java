...
public class DungeonMap {
	...
	private void scanForRooms(@NotNull MapState mapState) {
		...
		Set<PuzzleType> skipPuzzles = new HashSet<>();
		...
		for (DungeonRoom puzzle : this.puzzles) {
			final int puzzleId = puzzle.getPuzzleId();
		...
		Set<DungeonRoom> missingRooms = new HashSet<>();
		for (DungeonRoom dungeonRoom : puzzles.stream()
				.filter(DungeonRoom::isPuzzleDirty)
				.sorted(Comparator.comparingLong(DungeonRoom::getBecamePuzzleAt))
				.toList()) {
		...
		for (DungeonRoom missingRoom : missingRooms) {
			for (Pair<PuzzleType, Integer> knownPuzzle : knownPuzzles) {
				if (skipPuzzles.contains(knownPuzzle.getLeft())) {
					continue;
				}
				missingRoom.setPuzzleType(knownPuzzle.getLeft());
				break;
			}
		}
	}
