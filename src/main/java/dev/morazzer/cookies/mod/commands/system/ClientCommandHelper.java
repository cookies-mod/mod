...
public interface ClientCommandHelper {
	...
	default <S> RequiredArgumentBuilder<FabricClientCommandSource, S> argument(
			String name, ArgumentType<S> type) {
	...
	/**
	 * Provides a {@linkplain Command} instance that executes the {@linkplain Consumer} upon execution, and catches
	 * every exception that might occur.
	 *
	 * @param consumer The consumer.
	 * @return The command.
	 */
	default Command<FabricClientCommandSource> runAndWrap(
		...
		return context -> {
			try {
				...
				context.getSource().sendError(Text.translatable(TranslationKeys.UNEXPECTED_ERROR));
				return 1;
			}
			return 1;
		};
	...
	/**
	 * Ensures that the player is in a development environment, this should be used for all api interaction and for
	 * all potentially unsafe commands.
	 *
	 * @param source The command source.
	 * @return Whether the player is in a dev environment.
	 */
	default boolean ensureDevEnvironment(FabricClientCommandSource source) {
