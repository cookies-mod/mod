...
public enum ItemSearchCategories {
    ...
    ALL(new ItemStack(Items.GRASS_BLOCK), Text.literal("All"), Predicates.alwaysTrue()),
    ARMOR(
    ...
    MATERIAL(
        ...
        repositoryItem -> (repositoryItem.getUsedInRecipeAsIngredient().size() > 1 &&
    ...
    MINION(
        new ItemBuilder(Items.PLAYER_HEAD).setSkin(
            ...
            .build(),
        Text.literal("Minions"),
        repositoryItem -> repositoryItem.getInternalId().matches(".*GENERATOR_\\d+"));
    ...
    private static Predicate<RepositoryItem> createCategoryPredicate(String category) {
        return repositoryItem -> Optional.ofNullable(repositoryItem.getCategory())
