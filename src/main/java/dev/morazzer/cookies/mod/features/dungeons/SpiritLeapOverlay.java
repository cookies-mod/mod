...
public class SpiritLeapOverlay {
	...
	private void update(Slot slot) {
		...
		if (features.getCurrentInstance() == null) {
			dungeonPlayer = new DungeonPlayer(features.getCurrentInstance(), "meowora", "test", "mage");

		} else {
			dungeonPlayer = features.getCurrentInstance().getPlayer(slot.getStack().getName().getString());
		}
	...
	private void afterRender(Screen screen, DrawContext drawContext, int mouseX, int mouseY, float delta) {
		...
		for (int i = 0; i < this.players.length; i++) {
			...
			drawContext.fill(playerX, playerY, playerEndX, playerEndY, DungeonConfig.getInstance().spiritLeapFoldable.colorOption.getColorValue());
		...
		final DungeonMapRenderer mapRenderer;
		if (features.getCurrentInstance() == null) {
		...
		} else if (features.getCurrentInstance().getMapRenderer() == null) {
			return;
		} else {
			mapRenderer = features.getCurrentInstance().getMapRenderer();
		}
		if (mapRenderer == null) {
			return;
		}
