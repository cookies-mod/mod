...
public class CraftHelperPlacement extends ClientSideInventory {
    ...
    public CraftHelperPlacement() {
        ...
        getContents().fill(new ItemBuilder(Items.BLACK_STAINED_GLASS_PANE).hideTooltips().build());
    ...
    protected void init() {
        ...
        this.add(
            new PressableField(this.getX() - 9,
                this.getY() + 10,
                10,
                this.cookies$getBackgroundHeight() - 20,
                UNSELECTED_COLOR),
            CraftHelperLocation.LEFT_INVENTORY);
        ...
        this.add(
            new PressableField(this.width - 10,
                this.getY() + 10,
                10,
                this.cookies$getBackgroundHeight() - 20,
                UNSELECTED_COLOR),
            CraftHelperLocation.RIGHT);
        final PressableField field = getField(this.selected);
        field.setShouldRender(false);
    }

    private void add(PressableField field, CraftHelperLocation location) {
        field.setRunnable(click(location));
    ...
    private Runnable click(CraftHelperLocation craftHelperLocation) {
        return () -> {
            final PressableField field = getField(craftHelperLocation);
            ...
            field.setShouldRender(false);
            final PressableField previous = getField(this.selected);
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        context.drawTooltip(textRenderer,
            ...
            field.getX(),
            this.cookies$getY() + (this.cookies$getBackgroundHeight() / 2) - yOffset);
    }
